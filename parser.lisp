...
(defmethod parse-from (json (type gltf) gltf)
  (flet ((val (slot source type)
    ...
    (when (gethash "asset" json)
      (setf (asset gltf) (parse-from (gethash "asset" json) 'asset gltf)))
    ...
    (val 'shapes '("extensions" "KHR_implicit_shapes" "shapes") 'shape)
    (val 'physics-materials '("extensions" "KHR_physics_rigid_bodies" "physicsMaterials") 'physics-material)
    (val 'physics-joint-limits '("extensions" "KHR_physics_rigid_bodies" "physicsJointLimits") 'physics-joint-limit)
    (val 'collision-filters '("extensions" "KHR_physics_rigid_bodies" "collisionFilters") 'collision-filter)
